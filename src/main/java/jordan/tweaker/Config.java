...
public class Config {
    ...
    public static void loadConfig(Map<String, String> configProperties) {
        // Load minSpawnRadius first
        if (configProperties.containsKey("minSpawnRadius")) {
            putValue("minSpawnRadius", configProperties.get("minSpawnRadius"));
        }

        // Load maxSpawnRadius second
        if (configProperties.containsKey("maxSpawnRadius")) {
            putValue("maxSpawnRadius", configProperties.get("maxSpawnRadius"));
        }

        // Load other properties
        configProperties.forEach((k, v) -> {
            if (!k.equals("minSpawnRadius") && !k.equals("maxSpawnRadius")) {
                putValue(k, v);
            }
        });

        verifyConfig(); // Ensure all values are validated after loading all properties
    }
    ...
    private static void verifyConfig() {
        ...
        double maxSpawnRadius = configDoubles.getOrDefault("maxSpawnRadius", Double.MAX_VALUE);
        double minSpawnRadius = configDoubles.getOrDefault("minSpawnRadius", Double.MIN_VALUE);
        assertTrue(maxSpawnRadius >= minSpawnRadius,
                new NumberInvalidException("maxSpawnRadius must not be less than minSpawnRadius"));

        assertTrue(configIntegers.getOrDefault("quickSpawnHealth", 1) > 0,
    ...
    public static void putValue(String key, String value) {
        if (configDoubles.containsKey(key)) {
            putType(key, Double.parseDouble(value));
        } else if (configIntegers.containsKey(key)) {
            putType(key, Integer.parseInt(value));
        } else if (configBooleans.containsKey(key)) {
            putType(key, Boolean.parseBoolean(value));
        } else {
            throw new IllegalArgumentException("Unknown config key: " + key);
        }
        verifyConfig(); // Validate after setting any value
    }

    private static void putType(String key, double value) {
        configDoubles.put(key, value);
    }

    private static void putType(String key, int value) {
        configIntegers.put(key, value);
    }

    private static void putType(String key, boolean value) {
